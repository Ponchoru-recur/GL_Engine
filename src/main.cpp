...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
         ...
         //Uniform data does not vary and it is the same for every vertex
        glm::vec3 dominatingColor(0.0f, 1.0f, 0.0f);
        //This looks for the uniform ID in the vertex shader
        GLint dominatingColorUniformLocation = glGetUniformLocation(shaderProgram, "dominatingColor");
        GLint yFlipUniformLocation = glGetUniformLocation(shaderProgram, "yFlip");

        //Render Triangle
        glUseProgram(shaderProgram);
        glBindVertexArray(VAO);
        //Im putting the RGB color to the location of dominatingcolor
